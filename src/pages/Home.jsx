  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-screen bg-gray-900 text-white text-center p-6">
      ...
      <div className="flex flex-wrap justify-center gap-4 mt-10">
        {[
          // { label: "Enter Showroom", color: "blue", path: "/showroom" },
          { label: "See More Cars", color: "green", path: "/products" },
          { label: "Admin Login", color: "yellow", path: "/login" }
        ].map(({ label, color, path }) => (
          <motion.button
            key={label}
            className={`bg-${color}-600 hover:bg-${color}-700 px-6 py-3 rounded-lg text-lg font-medium shadow-md transition-all`}
